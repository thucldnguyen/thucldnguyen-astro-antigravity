...
<div class="chat-screenshot-pair">
  <figure>
    <img src="/blog/chat-gap-analysis-1.jpg" alt="Telegram screenshot: Thuc asks what info to add, noting the site is built for Vietnamese consumers" />
    <figcaption>The prompt. No specific asks — just audience context.</figcaption>
  </figure>
  <figure>
    <img src="/blog/chat-gap-analysis-2.jpg" alt="Telegram screenshot: agent's response — gap analysis with Zalo CTA and customer testimonials as high-priority items" />
    <figcaption>The response: a ranked gap analysis before touching a single file.</figcaption>
  </figure>
</div>
...
The original Gatsby site had no Zalo integration anywhere. The chain of reasoning — *this is a Vietnamese site → Vietnamese users use Zalo as their primary messaging channel → there should be a direct contact button → this doesn't exist → I should flag it* — happened without being prompted.

That's the gap between a tool that executes and a collaborator that thinks about the problem. It's the difference that matters in practice.
...
Then, cross-referencing a Báo Lâm Đồng newspaper article I'd shared, it found facts the documentary didn't have: the farm's output had grown to **20 tons/year** (the documentary said 10), and there was an **OCOP 3-star certification** — a Vietnamese government quality mark — that wasn't anywhere on the site. Those two facts propagated automatically across three blog posts and the homepage trust badge.
...
That last part is the one I keep coming back to: *knowing when to ask instead of guess*. For a small family business where credibility is everything, a confident wrong answer would have been worse than no answer.
