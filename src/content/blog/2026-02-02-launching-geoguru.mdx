...
tags: ["AI", "Game Dev", "Side Projects", "Experiment"]
---
...
I started out with ChatGPT to check whether static HTML sites based on Astro can host this game. Good news! They can. ChatGPT also gave me a perfect Monopoly-styled game image, and recommendations on where to download open-source soundtracks, flag images as well as country shapes to breathe life into the game.
...
---
## 2. Liquid-Glass UI and Mobile Fails
...
**The Snag**: On mobile, it was a disaster. The "Start Game" button was unclickable. The "Mute" toggle was pushed off-screen. The background image cropped awkwardly, hiding the cute "Welcome to GeoGuru" background.
...
Another snag: background music was not muted when the user navigated to other pages which is annoying when you try to read the blog posts. We fixed it by adding a logic to the game-exit state to mute the background music when the user navigates to other pages.
...
- **The "Tiny Island" problem**: Some countries especially islands are just too small so their coast/border lines are too coarse/jagged. I had to refine the borders by setting the simplifcation percentage to 100% (no simplifcation at all) for those countries.

![Squashed Canada](../../assets/squashed-ca.jpg)
...
3.  **Automated Styling**: We piped the output through `mapshaper` to apply a uniform "Yellow Fill + Black Border" style that pops against the dark UI.
...
Fun note: AI can make silly mistakes too. In the example below, it mistakenly set the simplification percentage to 0.5%, resulting in a blank image for "Saint Pierre and Miquelon"

![AI mistake](../../assets/AI-bug.png)
...
This freed me up to focus on the **User Experience**: the difficulty progression, the graphics, the sound effects, the animations, and the overall "juice" of the game.
